...
function BalochistanProvincialBody() {
  const data = balochistanProvincialData;
    ...
    <main className="w-full bg-white">
      ...
      {featuredProfile && (
        <section className="max-w-[1200px] mx-auto px-4 pt-10 pb-10">
          <Link
            to={`/branches/provincial/balochistan/profile/${featuredProfile.id}`}
      ...
      <section className="max-w-[1200px] mx-auto px-4 pb-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
          {otherProfiles.map((profile) => (
            <Link
              key={profile.id}
              to={`/branches/provincial/balochistan/profile/${profile.id}`}
...
export default BalochistanProvincialBody;