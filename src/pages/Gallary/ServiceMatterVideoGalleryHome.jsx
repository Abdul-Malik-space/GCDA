...
function ServiceMatterVideoGalleryHome() {
  const navigate = useNavigate();

  const openAlbum = (event) => {
    navigate(`/gallery/service-matter-videos/${event.id}`);
  };
    ...
    <div className="min-h-screen bg-gray-50 w-full pb-20 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        <AnimatePresence mode="wait">
          ...
          >
            {serviceMatterVideoData && serviceMatterVideoData.length > 0 ? (
              serviceMatterVideoData.map((event) => (
                <motion.div
                  key={event.id}
              ...
              <div className="col-span-full text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
                <p className="text-gray-400 font-semibold">
                  No service matter videos available.
                </p>
              </div>
            )}
          </motion.div>
...
export default ServiceMatterVideoGalleryHome;