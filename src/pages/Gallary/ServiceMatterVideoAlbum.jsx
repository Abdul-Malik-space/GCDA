...
import { serviceMatterVideoData } from "../../data/serviceMatterVideoData";
...
const ServiceMatterVideoAlbum = () => {
  const { eventId } = useParams();
  const navigate = useNavigate();

  const event = useMemo(() => {
    return serviceMatterVideoData.find((item) => item.id === eventId);
  }, [eventId]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
          <h1 className="text-2xl font-black text-gray-800 mb-3">
            Service matter video album not found
          </h1>

          <button
            onClick={() => navigate("/gallery/service-matter-videos")}
            className="px-5 py-2.5 rounded-xl bg-[#0F765E] text-white font-bold"
          >
            Back to Service Matter Videos
          </button>
        </div>
      </div>
    );
          ...
          <div className="relative w-full max-w-5xl">
            <div className="bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10">
              ...
              <div className="bg-white px-5 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                >
                  ← Return to Service Matter Videos
                </button>
              </div>
            </div>
          </div>
...
export default ServiceMatterVideoAlbum;